...
def respond_to_command(
    ...
    run_function = safe_get(routing.COMMAND_MAPPER, safe_get(body, "command"))
    if run_function:
        run_function(
            body=body,
            client=client,
        ...
        logger.warning("No command found for {}".format(safe_get(body, "command")))
...
def respond_to_view(ack, body, client, logger, context):
    ...
    if run_function:
        ...
        logger.warning("No view found for {}".format(safe_get(body, "view", "callback_id")))


def respond_to_action(ack, body, client, logger, context):
    ack()
    logger.debug("body is {}".format(body))
    trigger_id = safe_get(body, "trigger_id")
    ...
    run_function = safe_get(routing.ACTION_MAPPER, safe_get(body, "actions", 0, "action_id"))
    if run_function:
        run_function(
            body=body,
            client=client,
        ...
        logger.warning("No action handler found for {}".format(safe_get(body, "actions", 0, "action_id")))
...
#             text="Editing this backblast is only allowed for the Q(s), the original poster, "
#                   "or your local Slack admins."
...
if constants.LOCAL_DEVELOPMENT:
    ...
    ACTION_KWARGS["ack"] = lambda ack: ack()
    ACTION_KWARGS["lazy"] = [respond_to_action]
...
app.action(MATCH_ALL_PATTERN)(*ACTION_ARGS, **ACTION_KWARGS)
